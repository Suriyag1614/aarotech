        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {industries.map((industry) => (
            <Link href={`/industries/${industry.id}`} key={industry.id} className="block group">
              <Card className="h-full border-none shadow-sm group-hover:shadow-lg transition-all duration-300 group-hover:-translate-y-1">
                <CardContent className="p-6 sm:p-8 flex flex-col h-full">
                  <h3 className="text-2xl font-bold mb-8 text-foreground group-hover:text-primary transition-colors">{industry.title}</h3>
                  <div className="space-y-6 flex-1">
                    <div className="flex items-start bg-destructive/5 p-4 rounded-lg border border-destructive/10">
                      <XCircle className="w-5 h-5 text-destructive shrink-0 mt-0.5 mr-3" />
                      <div>
                        <span className="block text-sm font-bold text-foreground mb-1.5 uppercase tracking-wide">The Bottleneck</span>
                    ...
                    <div className="flex items-start bg-green-50 p-4 rounded-lg border border-green-100">
                      ...
                      <div>
                        <span className="block text-sm font-bold text-foreground mb-1.5 uppercase tracking-wide">Our Fix</span>
                        <p className="text-sm text-muted-foreground leading-relaxed">{industry.solution}</p>
                      </div>
                    </div>
                  </div>
                  <div className="mt-8 pt-6 border-t flex items-center text-primary font-medium">
                    View Strategy
